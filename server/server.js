...
app.get("/api/weather", async (req, res) => {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/current.json?key=7f47f9ba1d7d440a8ca25252230707&q=77840`,
    );
...
const client = new OAuth2Client(
  "402590880820-dmqaqh91m4fnmie6h1680mslj3bfkbt3.apps.googleusercontent.com",
);
...
app.post("/api/customer/google-signin", async (req, res) => {
  ...
  try {
    ...
    const insertResult = await pool.query(
      "INSERT INTO customer (customer_id, customer_name, customer_email) VALUES ($1, $2, $3) RETURNING customer_id",
      [newId, googleName, googleEmail],
    );

    res.json({ customer_id: insertResult.rows[0].customer_id });
  } catch (error) {
...
app.post("/api/customer/login", async (req, res) => {
  ...
  try {
    // Check if email and password match an existing customer
    const result = await pool.query(
...
app.post("/api/customer/register", async (req, res) => {
  ...
  try {
    ...
    const idResult = await pool.query("SELECT MAX(customer_id) FROM customer");
    const id = idResult.rows[0].max ? idResult.rows[0].max + 1 : 1; // Handle case where there are no customers
